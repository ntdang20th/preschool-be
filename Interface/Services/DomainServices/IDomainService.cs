...
namespace Interface.Services.DomainServices
{
    public interface IDomainService<E, T> where E : Entities.DomainEntities.DomainEntities where T : BaseSearch
    {
        ...
        Task Validate(E item);
        Task AddItem(E model);
        Task<E> AddItemWithResponse(E model);
        Task UpdateItem(E model);
        Task<E> UpdateItemWithResponse(E model);
        ...
        Task<bool> AnyAsync(Expression<Func<E, bool>>[] expressions);
        Task<bool> AnyAsync(Expression<Func<E, bool>> expressions);
        Task<int> CountAsync(Expression<Func<E, bool>>[] expressions);
        Task<int> CountAsync(Expression<Func<E, bool>> expressions);
    }
}
