...
{
    public interface IDomainRepository<T> where T : Entities.DomainEntities.DomainEntities, new()
    {
        IQueryable<T> GetQueryable();
        Task<T> Validate(Guid id);
        ...
        Task<List<T>> GetDataExport(string query, SqlParameter[] parameters);
        Task<T> GetSingleRecordAsync(string query, SqlParameter[] parameters);
    }
}
