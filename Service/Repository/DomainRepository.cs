...
{
    public class DomainRepository<T> : IDomainRepository<T> where T : Entities.DomainEntities.DomainEntities, new()
    {
        ...
        public virtual async Task<T> Validate(Guid id)
        {
            return await Context.Set<T>().FirstOrDefaultAsync(x=>x.deleted == false && x.id == id);
        }
        ...
        public async Task<T> GetSingleRecordAsync(string query, SqlParameter[] parameters)
        {
            using (var command = Context.Database.GetDbConnection().CreateCommand())
            {
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        return MapDataToEntity<T>(reader);
                    }
                }
            }
            return default(T);
        ...
        public async Task<List<T>> GetDataExport(string query, SqlParameter[] parameters)
        {
            ...
            using (var command = Context.Database.GetDbConnection().CreateCommand())
            {
                ...
                using (var reader = await command.ExecuteReaderAsync())
                {
                    ...
                    {
                        var item = MapDataToEntity<T>(reader);
                        results.Add(item);
                    }
                }
            }
        ...
        private T MapDataToEntity<T>(DbDataReader reader) where T : new()
        {
            T item = new T();
            var type = typeof(T);
            ...
            for (int i = 0; i < reader.FieldCount; i++)
            {
                ...
                var propInfo = type.GetProperty(columnName, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
